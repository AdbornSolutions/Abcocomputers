...
class x {
  ...
  #p() {
  if (this.#e.canvas instanceof HTMLCanvasElement) {
  ...
  if (!(this.canvas instanceof HTMLCanvasElement)) {
    throw new Error('Three: Valid canvas element not found');
  }
...
class Z extends d {
  constructor(e, t = {}) {
    const i = { ...X, ...t };
    const s = new z();
    const pmremGenerator = new p(e);
    const n = pmremGenerator.fromScene(s, 0.04).texture;
    const o = new g();
...
function createBallpit(e, t = {}) {
  const i = new x({
    canvas: e,
    size: 'parent',
    rendererOptions: { antialias: true, alpha: false, precision: 'mediump' }
  });
  let s;
